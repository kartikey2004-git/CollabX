...
import { BookMarked, ExternalLink, Plus, TrendingUp } from "lucide-react";
...
export function TechReadList() {
  ...
  const { data, isPending, isError, error, refetch, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useTechReads({
      ...
      isTrending: trendingOnly ? true : undefined,
    });
    ...
    <div className="space-y-6">
      ...
      {!isPending && !isError && techReads.length === 0 && (
        <EmptyState
          ...
          description={
            trendingOnly
              ? "Nothing is marked trending right now."
              : "Nothing has been curated in this category yet."
          }
        />
      )}
