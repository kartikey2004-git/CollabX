...
import type { Role, TechRead } from "@repo/database";
...
export const techReadService = {
  ...
  // Public, PUBLISHED-only listing. When `isTrending: true` is requested, sort by trendingScore
  // regardless of the client's `sortBy` — this endpoint IS the "Trending Tech Reads" feed in that
  // mode, and trendingScore is the only sort order that makes sense for it.
  list(query: ListTechReadsQuery) {
    ...
    return getOrSetCache(cacheKey, LIST_CACHE_TTL_SECONDS, () =>
      techReadRepository.findPublished({
        ...
        cursor: query.cursor,
        limit: query.limit,
        sortBy: query.isTrending ? "trendingScore" : query.sortBy,
        sortOrder: query.isTrending ? "desc" : query.sortOrder,
      }),
    );
  },
  ...
  // ADMIN-only, and restricted at the schema layer to isTrending/trendingScore ONLY — see
  // updateTechReadTrendingSchema's comment for why this one direct-update endpoint is safe despite
  // Article/TechRead otherwise having no general "edit in place" path.
  async updateTrending(id: string, input: UpdateTechReadTrendingInput): Promise<TechRead> {
    const techRead = await techReadRepository.findById(id);
    if (!techRead) {
      throw new NotFoundError("Tech read not found");
    }
    const updated = await techReadRepository.updateTrending(id, input);
    // Invalidate immediately rather than waiting out the TTL — an admin tuning the "Trending Tech
    // Reads" feed expects the change to show up right away, not up to 30s later.
