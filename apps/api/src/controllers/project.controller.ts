...
import type { CreateProjectInput, ListProjectsQuery, UpdateProjectInput } from "@repo/validation";
...
export const projectController = {
  async create(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Grab the data the user sent in the request body like name, description
  ...
  async list(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Grab query params like limit, sortBy, sortOrder, cursor which is used for pagination
      ...
      const { items, nextCursor, hasMore } = await projectService.list(workspaceId, query);
  ...
  async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Get the new data like name, description to update the project with, from the request body
