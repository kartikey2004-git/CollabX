...
import type { Category, ContentStatus, Prisma, TechRead } from "@repo/database";
...
export type TechReadSortField = "createdAt" | "updatedAt" | "title" | "publishedAt" | "trendingScore";
...
export const techReadRepository = {
  ...
  // Public listing: PUBLISHED-only. `isTrending: true` narrows to the "Trending Tech Reads" feed —
  // the service overrides sortBy to trendingScore in that case (see tech-read.service.ts).
  ...
  // ADMIN-only "toggle trending" — restricted at the validation layer to only these two fields
  // (see updateTechReadTrendingSchema), so this repository method mirrors that restriction rather
  // than accepting an arbitrary partial update.
  updateTrending(
    id: string,
    data: { isTrending?: boolean; trendingScore?: number },
  ): Promise<TechRead> {
    return db.techRead.update({ where: { id }, data });
  },
