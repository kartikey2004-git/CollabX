...
// Re-export every generated model type/enum/the Prisma namespace (Role, Article, Prisma.*, etc.)
// so callers can do `import db, { type Role } from "@/lib/db"` from one place, mirroring the old
// @repo/database package's public API (its src/index.ts did the same re-export).
