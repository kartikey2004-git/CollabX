...
// The information we need to remember "where we left off" in a paginated list: the value we last sorted by, + that row's id (to break ties).
...
// decodeCursor reverses encodeCursor: takes the cursor string from the request and turns it back into the { value, id } which is needed to query the next page.
...
export function buildKeysetWhere(
  ...
  // Ascending order lookup in database for "greater than" the cursor; descending lookup in database for "less than".
  const op = sortOrder === "asc" ? "gt" : "lt";
  return {
    OR: [{ [sortBy]: { [op]: cursor.value } }, { [sortBy]: cursor.value, id: { [op]: cursor.id } }],
  };
}
...
In plain English: "give me all the rows where createdAt > lastSeenCreatedAt, OR (createdAt is exactly equal but id > lastSeenId)." The second branch handles ties — e.g. two projects created in the same millisecond — so no row gets skipped or duplicated across pages.
...
Why this beats OFFSET/LIMIT:
  ...
  - If a row is inserted while someone's paging through, offset-based paging can skip or repeat rows; cursor-based paging can't, because It's based on the values of a real row, not its position in the result set.
